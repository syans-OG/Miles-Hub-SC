--[[
    ...
    Structure identical to diagnostic (which worked).
...
local function Section(p, t)
    ...
    s.Text = "  "..t; s.TextColor3 = Color3.fromRGB(130, 90, 220)
...
local function Paragraph(p, t, c)
    ...
    pp.Size = UDim2.new(1, 0, 0, 36); pp.BackgroundColor3 = Color3.fromRGB(35, 35, 50)
    pp.Text = "  "..t.."\n  "..c; pp.TextColor3 = Color3.fromRGB(160, 160, 170)
    pp.Font = Enum.Font.Gotham; pp.TextSize = isMobile and 7 or 9; pp.TextWrapped = true
    pp.TextXAlignment = Enum.TextXAlignment.Left; pp.TextYAlignment = Enum.TextYAlignment.Top
...
local function Toggle(p, n, d, cb)
    ...
    l.Size = UDim2.new(1, -42, 1, 0); l.BackgroundTransparency = 1; l.Text = "  "..n
...
local function Btn(p, n, cb)
    ...
    b.Text = "  "..n; b.TextColor3 = Color3.fromRGB(240, 240, 240)
...
local function Slider(p, n, mn, mx, d, sf, cb)
    ...
    l.Text = "  "..n; l.TextColor3 = Color3.fromRGB(240, 240, 240); l.Font = Enum.Font.GothamSemibold
    l.TextSize = isMobile and 7 or 10; l.TextXAlignment = Enum.TextXAlignment.Left
    local v2 = Instance.new("TextLabel", f)
    v2.Size = UDim2.new(0.4, 0, 0, 14); v2.Position = UDim2.new(0.58, 0, 0, 2); v2.BackgroundTransparency = 1
    v2.Text = tostring(d)..(sf or""); v2.TextColor3 = Color3.fromRGB(130, 90, 220)
    v2.Font = Enum.Font.GothamBold; v2.TextSize = isMobile and 7 or 10; v2.TextXAlignment = Enum.TextXAlignment.Right
    ...
    UserInputService.InputChanged:Connect(function(i)
        if drag and (i.UserInputType == Enum.UserInputType.MouseMovement or i.UserInputType == Enum.UserInputType.Touch) then
            ...
            cur = math.floor(mn + p2 * (mx - mn)); cur = math.clamp(cur, mn, mx)
            local np = (cur - mn) / (mx - mn)
            fill.Size = UDim2.new(np, 0, 1, 0); knob.Position = UDim2.new(np, -7, 0.5, -7)
            v2.Text = tostring(cur)..(sf or""); pcall(cb, cur)
        end
    end)
end
...
for r, nm in ipairs({"Celestial Chicken","Dark Lord Rooster","Phoenix Chicken","Quantum Chicken","Lava Rooster"}) do
    Paragraph(home, "#"..r.." "..nm, "Power: Top Tier")
end
Section(home, "⚡ QUICK")
...
Btn(home, "🌐 Server Hop", function()
    pcall(function()
        ...
        if d and d.data then for _, s in ipairs(d.data) do if s.playing < s.maxPlayers and s.id ~= game.JobId then game:GetService("TeleportService"):TeleportToPlaceInstance(game.PlaceId, s.id, LP); return end end end
    end)
    N("No empty server")
end)
...
Section(sett, "⚡ PERFORMANCE")
Toggle(sett, "FPS Booster", false, function(v)
    Flags.FPSBooster = v
    pcall(function()
...
Section(sett, "🏃 MOVEMENT")
Toggle(sett, "Inf Jump", false, function(v) Flags.InfJump = v end)
Toggle(sett, "NoClip", false, function(v) Flags.NoClip = v end)
Slider(sett, "WalkSpeed", 16, 250, 16, " Spd", function(v) Flags.WalkSpeed = v end)
Slider(sett, "JumpPower", 50, 300, 50, " Pwr", function(v) Flags.JumpPower = v end)
...
-- ═══ FEATURES (ALL in pcall — like diagnostic) ═══
...
N("All features loaded! ⚡")
print("[Miles-HUB] v2.2 loaded — NO RunService, NO VirtualUser")
