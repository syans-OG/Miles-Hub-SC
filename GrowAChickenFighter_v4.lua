--[[
    ⚡ Miles-HUB v4.0 — Grow A Chicken (Anti-Cheat Safe)
    
    FIXES:
    - GUI parent: gethui() → CoreGui → PlayerGui (with validation)
    - NO RunService connections at all
    - NO VirtualUser usage
    - Slower loops with random delays
    - Better error handling
    
    Usage: paste into executor, execute
...
Notify("v4.0 starting...")

-- ═══ STEP 2: GUI CREATION (with proper parent chain) ═══
local isMobile = UserInputService.TouchEnabled and not UserInputService.KeyboardEnabled
local guiW = isMobile and 280 or 400
local guiH = isMobile and 350 or 480

local gui = Instance.new("ScreenGui")
gui.Name = "MilesHub_v4"
gui.ResetOnSpawn = false
gui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
gui.DisplayOrder = 999999

-- Parent chain: try each until one works
local guiParent = nil
local parentAttempts = {
    function() return gethui and type(gethui) == "function" and gethui() end,
    function() return game:GetService("CoreGui") end,
    function() return LP:WaitForChild("PlayerGui", 5) end,
}

for _, getParent in ipairs(parentAttempts) do
    local ok, parent = pcall(getParent)
    if ok and parent then
        local attachOk = pcall(function() gui.Parent = parent end)
        if attachOk and gui.Parent == parent then
            guiParent = parent
            break
        end
    end
...
if not guiParent then
    warn("[Miles-HUB] CRITICAL: Could not attach GUI to any parent!")
    Notify("ERROR: GUI attach failed!")
    return
end

Notify("GUI attached to: " .. guiParent:GetFullName())
...
Btn(home, "Rejoin Server", function() 
...
pcall(function()
    ...
    if not floatParent then pcall(function() floatParent = game:GetService("CoreGui") end) end
    if not floatParent then floatParent = LP:WaitForChild("PlayerGui", 5) end
    
    if floatParent then
        fg.Parent = floatParent
...
Notify("Floating button ready")

-- ══════════════════════════════════════════════════════════
-- STEP 3: SAFE FEATURES (NO RunService, NO VirtualUser)
-- ══════════════════════════════════════════════════════════

-- InfJump (SAFE: uses JumpRequest event)
pcall(function()
...
Notify("InfJump ready")

-- Speed/Jump/NoClip (SAFE: uses task.spawn with slow loop)
pcall(function()
    task.spawn(function()
        while task.wait(0.5) do  -- Slow loop to avoid detection
            pcall(function()
                local c = LP.Character
                if c then
                    local h = c:FindFirstChildOfClass("Humanoid")
                    if h then
                        ...
                        if Flags.JumpPower > 50 then 
                            h.JumpPower = Flags.JumpPower 
                        end
...
Notify("Movement features ready")

-- Anti-AFK (SAFE: uses Idled event, no VirtualUser)
pcall(function()
    LP.Idled:Connect(function()
        if Flags.AntiAFK then
            -- Simple mouse click without VirtualUser
            pcall(function()
...
Notify("Anti-AFK ready")

-- ═══ BACKGROUND LOOPS (SAFE: slow, random delays) ═══
...
task.spawn(function()
    while task.wait(1) do  -- Check every second
        if Flags.AutoTrain then
            pcall(function() Fire("Train") end)
        end
        if Flags.AutoPunch then
            pcall(function() Fire("Punch") end)
        end
...
task.spawn(function()
    while task.wait(2) do
        if Flags.AutoCollectScrap then
            pcall(function() Fire("CollectScrap") end)
        end
    end
end)
...
-- ═══ DONE ═══
Notify("ALL LOADED! RightShift to toggle GUI")
print("[Miles-HUB] v4.0 loaded successfully!")
print("[Miles-HUB] NO RunService, NO VirtualUser - SAFE MODE")
