--[[ Miles-HUB v6.0 - Grow A Chicken - PURE ASCII, no unicode ]]
...
-- GUI - parented DIRECTLY to PlayerGui (always works)
...
-- FIXED Fire: args captured in table, NO ... inside closure
...
-- Anti-AFK (safe: Idled event only)
pcall(function()
...
task.spawn(function()
    while task.wait(1) do
        if Flags.AutoHatch then
            local ok = pcall(function() Fire("HatchEgg", Flags.SelectedEgg, 1) end)
...
task.spawn(function()
    while task.wait(2) do
        if Flags.AutoCollectScrap then pcall(function() Fire("CollectScrap") end) end
    end
end)
