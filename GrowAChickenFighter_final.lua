--[[
    ⚡ Miles-HUB v2.2 — FULL FEATURES (Anti-Cheat Safe)
    
    Pola yang TERBUKTI AMAN dari V3:
    - JumpRequest untuk InfJump ✅
    - Button click untuk Speed/Jump ✅
    - TIDAK ada RunService.Stepped/Heartbeat connections ❌
    - TIDAK ada VirtualUser ❌
    
    Semua fitur berjalan via task.spawn loops (bukan RunService).
]]
...
-- ═══ Auto Loops (task.spawn — NO RunService) ═══
...
task.spawn(function() while true do if Flags.AutoCollectEggs then pcall(function() Fire("CollectEgg") end) end task.wait(0.5) end end)
task.spawn(function() while true do if Flags.AutoClaimIncubator then pcall(function() for s = 1, 10 do Fire("ClaimIncubator", s) end end) end task.wait(1) end end)
...
Section(home, "⚡ QUICK")
Btn(home, "⚡ Equip Best", function() Fire("EquipBest"); N("Equipped!") end)
...
for _, en in ipairs({"Basic Egg","Forest Egg","Desert Egg","Magma Egg","Cyber Egg","Void Egg"}) do
    Btn(egg, "🥚 "..en, function() Flags.SelectedEgg = en; Fire("HatchEgg", en, 1); N("Hatch: "..en) end)
end
Section(egg, "📦 COLLECT")
...
Btn(egg, "💵 Sell All", function() Fire("SellChicken", "All"); N("Sold!") end)
Toggle(egg, "Auto Fuse", false, function(v) Flags.AutoFuse = v end)
Btn(egg, "🧬 Fuse Now", function() Fire("FuseChicken", "AutoFuseDuplicates", true); N("Fuse sent!") end)
...
Btn(tower, "⚡ Rebirth Now", function()
    ...
    Fire("Rebirth", "DoRebirth"); N("Rebirth done!")
end)
...
Toggle(settings, "Anti-AFK", true, function(v) end)
